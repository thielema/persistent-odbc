...
getColumn getter tname [ PersistByteString cname
    ...
    runExceptT $ do
      ...
      defaultConstraintName_ <- case defaultConstraintName' of
                    PersistNull   -> return Nothing
                    PersistText t -> return $ Just $ ConstraintNameDB t
                    PersistByteString bs ->
                      case T.decodeUtf8' bs of
                        Left exc -> fail $ "Invalid default constraintname: " ++
                                           show defaultConstraintName' ++ " (error: " ++
                                           show exc ++ ")"
                        Right t  -> return $ Just $ ConstraintNameDB t
...
findAlters tblName allDefs col@(Column name isNull type_ def _generated defConstraintName _maxLen ref) cols =
    tracex ("\n\n\nfindAlters tablename="++show tblName++ " name="++ show name++" col="++show col++"\ncols="++show cols++"\n\n\n") $
      case filter ((name ==) . cName) cols of
        ...
        Column _ isNull' type_' def' _generated defConstraintName' _maxLen' ref':_ ->
            let -- Foreign key
                refDrop =
                    case (ref == ref', ref') of
                        (False, Just cRef) ->
                ...
                modDef | def == def' = []
                       | otherwise   = tracex ("\n\nfindAlters modDef col=" ++ show col ++ " name=" ++ show name ++ " def=" ++ show def ++ " def'=" ++ show def' ++ " defConstraintName=" ++ show defConstraintName++" defConstraintName'=" ++ show defConstraintName' ++ "\n\n") $
                                        case def of
                                         Nothing -> [NoDefault $ maybe (error $ "expected a constraint name col="++show name) id defConstraintName']
                                         Just s -> if cmpdef def def' then [] else [Default name $ T.unpack s]
...
showAlter table (Change (Column n nu t def generated defConstraintName maxLen _ref)) =
    concat
    [ "ALTER TABLE "
    , escapeDBName table
    , " ALTER COLUMN "
...
showAlter table (NoDefault defConstraintName) =
    concat
    [ "ALTER TABLE "
    , escapeDBName table
    , " DROP CONSTRAINT "

...
refName (EntityNameDB table) (FieldNameDB column) =
    ConstraintNameDB $ T.take 30 $ T.concat [table, "_", column, "_fkey"]
...
pkeyName (EntityNameDB table) =
    ConstraintNameDB $ T.take 30 $ T.concat [table, "_pkey"]
...
getSeqNameEscaped :: EntityNameDB -> String
getSeqNameEscaped d = escapeDBName $ ConstraintNameDB $ getSeqNameUnescaped d

getSeqNameUnescaped :: EntityNameDB -> Text
getSeqNameUnescaped (EntityNameDB s) = "seq_" <> s <> "_id"
